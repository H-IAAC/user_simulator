...
{
    [CreateAssetMenu(menuName ="HIAAC/ScriptableList/Geo List")]
    public class GeoSList : Vector3SList
    {
        [SerializeField] MapReference mapReference;

        protected override Vector3 processGettedItem(Vector3 item)
        {
            //lat = x long = y
            double lat = item.x;
            double lon = item.y;

            AbstractMap map = mapReference.map;
